...
class ViewBlacklistsTetCase(unittest.TestCase):
  ...
  def test_consult_email_registered(self):
    ...
    email_register = Blacklist(email=email,app_uuid="ASDFVGYHNMJIKLOPIGG",blocked_reason="El usuario usa lengiaje violento y grosero contra otros usuarios.",ip="184.4.0.160",created_at="2022-04-03 13:59:10.307")
  ...
  def test_consult_email_unregistered(self):
    ...
    token = "eyJ0eXAiOiJKV1QiLCJhbGciOiJIUzI1NiJ9..MpR0dkXB5zadTI-4FEYV5pr4ofx5e7IJkmJdTsgrrG8"
    addEmailResponse = self.client.get(f'api/blacklists/{email}',content_type='application/json', headers=dict(Authorization='Bearer {0}'.format(token)))
    self.assertEqual(addEmailResponse.status_code, 404)
